...
public String getAuthor() {
  ...
  public void run(double deltaMs) {
    ...
    for (LXPoint point : model.points) {
      ...
      float dist = (float) LXUtils.distance(point.x, point.y, xPos, yPos); 
      //float falloff = 100 / max(20*INCHES, 2*size - .5*dist);
      //float b = 100 - falloff * LXUtils.wrapdistf(radix, pos * size, size);
      float b = Math.abs(((dist + radix + pos * size) % size) * dMult - 1);
      setColor(point.index, (b > 0) ? LXColor.gray(b*b*100) : 0);
    }
  }
}