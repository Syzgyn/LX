...
import stratovo.StratovoUtils;

@LXCategory(LXCategory.COLOR)
public class ColorPlasma extends LXPattern {
	...
	public void run(double deltaMs) {
		...
		for (LXPoint point : model.points) {
			...
			red = StratovoUtils.map(sinTable.sin(shade*PI), -1, 1, 0, brightness);
			green =  StratovoUtils.map(sinTable.sin(shade*PI+(2*cosTable.cos(movement*490))), -1, 1, 0, brightness); //*cos(movement*490) makes the colors morph over the top of each other 
			blue = StratovoUtils.map(sinTable.sin(shade*PI+(4*sinTable.sin(movement*300))), -1, 1, 0, brightness);
	...
	public static class PlasmaGenerator {
		...
		{
			return sinTable.sin( (float) (( ( p.y / ymax / size) * Math.sin( movement /66 )) + (p.z / zmax / size) * (Math.cos(movement / 100)))  ) ;
		}
